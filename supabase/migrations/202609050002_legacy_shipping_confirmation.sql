...
alter table public.sales
  add column if not exists legacy_shipping_confirmation text,
  add column if not exists legacy_shipping_confirmed_at timestamptz,
  add column if not exists legacy_shipping_confirmed_by uuid references public.profiles(id);
...
alter table public.sales add constraint sales_legacy_shipping_confirmation_check check (
  (legacy_shipping_confirmation is null and legacy_shipping_confirmed_at is null and legacy_shipping_confirmed_by is null)
  or (legacy_shipping_confirmation = 'pending' and legacy_shipping_confirmed_at is null and legacy_shipping_confirmed_by is null)
  or (legacy_shipping_confirmation in ('sent', 'not_sent') and legacy_shipping_confirmed_at is not null and legacy_shipping_confirmed_by is not null)
);

create index if not exists sales_legacy_shipping_confirmation_idx
  on public.sales(organization_id, legacy_shipping_confirmation, sale_date, id)
  where deleted_at is null;

create or replace function public.guard_legacy_shipping_confirmation_write()
returns trigger language plpgsql set search_path = public as $$
begin
  if (new.legacy_shipping_confirmation, new.legacy_shipping_confirmed_at, new.legacy_shipping_confirmed_by)
     is distinct from
     (old.legacy_shipping_confirmation, old.legacy_shipping_confirmed_at, old.legacy_shipping_confirmed_by)
...
before update of legacy_shipping_confirmation, legacy_shipping_confirmed_at, legacy_shipping_confirmed_by
...
begin
  ...
  if normalized_confirmation not in ('pending', 'sent', 'not_sent') then
    raise exception 'invalid_legacy_shipping_confirmation';
  end if;
  ...
  if sale_row.organization_id not in (select public.current_user_org_ids())
     or not public.has_org_permission(sale_row.organization_id, 'sales.edit') then
    raise exception 'forbidden';
  end if;
  if sale_row.updated_at is distinct from p_expected_updated_at then raise exception 'stale_sale'; end if;

  -- "pending" is intentionally stored as NULL so existing rows need no backfill.
  ...
  if normalized_confirmation = 'not_sent' and (
    ...
    raise exception 'operational_shipment_confirmed';
  end if;

  if sale_row.legacy_shipping_confirmation is not distinct from stored_confirmation then
    return jsonb_build_object('sale_id', sale_row.id, 'confirmation', coalesce(sale_row.legacy_shipping_confirmation, 'pending'),
      'confirmed_at', sale_row.legacy_shipping_confirmed_at, 'confirmed_by', sale_row.legacy_shipping_confirmed_by,
  ...
  returning jsonb_build_object('confirmation', coalesce(legacy_shipping_confirmation, 'pending'),
    'confirmed_at', legacy_shipping_confirmed_at, 'confirmed_by', legacy_shipping_confirmed_by), updated_at
  into after_state, changed_at;
  ...
  return jsonb_build_object('sale_id', sale_row.id, 'confirmation', after_state ->> 'confirmation',
    'confirmed_at', after_state ->> 'confirmed_at', 'confirmed_by', after_state ->> 'confirmed_by',
    'updated_at', changed_at, 'changed', true);
end;
$$;

revoke all on function public.set_legacy_shipping_confirmation(uuid, text, timestamptz, text) from public, anon, authenticated;
grant execute on function public.set_legacy_shipping_confirmation(uuid, text, timestamptz, text) to authenticated;
