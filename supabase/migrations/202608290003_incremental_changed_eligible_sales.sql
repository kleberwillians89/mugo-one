...
-- ==========================================================================
-- IMPORTAÇÃO INCREMENTAL — CHANGED comercial em venda elegível
-- --------------------------------------------------------------------------
-- As migrations históricas NÃO são editadas. Esta republica a RPC
-- preservando TODAS as invariantes:
--   - somente service_role + admin da organização;
--   - idempotência por file_hash;
--   - inventory_allocation_eligible = false em toda venda importada;
--   - zero escrita de estoque / movimento de compra / shipment / preparação;
--   - checagem de totais de classificação; changed sem match rejeitado;
--   - new_safe que viraria duplicata rejeitado.
--
-- Correções seguras acumuladas:
--   - CHANGED comercial é permitido em venda elegível; triggers seguem ativos;
--   - IDs canônicos resolvidos no staging são usados e validados por tenant;
--   - catálogo/cliente só são criados quando não há ID resolvido;
--   - CHANGED altera somente campos presentes em proposed_changes;
--   - NEW preserva multiplicidade legítima dentro do lote corrente;
--   - a proteção contra duplicata pré-existente e a idempotência permanecem.
-- ==========================================================================
...
begin
  ...
  for r in select item from jsonb_array_elements(p_rows) item order by (item->>'source_row')::integer loop
    ...
    insert into public.import_rows(
      ...
      jsonb_build_object('client',r.item->>'client','sale_date',r.item->>'date','perfume',r.item->>'perfume',
        ...
        'payment_status',r.item->>'payment_status','split_completed_at',r.item->>'split_completed_at','classification',r.item->>'classification'),
  ...
  from jsonb_array_elements(p_rows) item where item->>'classification'='new_safe'
    and coalesce(item->>'resolved_client_id','')=''
  ...
  from jsonb_array_elements(p_rows) item where item->>'classification'='new_safe'
    and coalesce(item->>'resolved_perfume_id','')=''
  ...
  loop
    ...
    if not found then raise exception 'unsafe_changed_match'; end if;
    v_before:=jsonb_build_object('payment_status',v_sale.payment_status,'payment_method',v_sale.payment_method,
      ...
      'split_completed_at',v_sale.split_completed_at);
    update public.sales set
      payment_status=case when r.item->'proposed_changes'?'payment_status' then (r.item->>'payment_status')::public.payment_status else payment_status end,
      payment_method=case when r.item->'proposed_changes'?'payment_method' then nullif(r.item->'raw'->>'FORMA DE PAGAMENTO','') else payment_method end,
      paid_at=case when r.item->'proposed_changes'?'paid_at' then nullif(r.item->>'paid_at','')::date else paid_at end,
      shipped_at=case when r.item->'proposed_changes'?'shipped_at' then nullif(r.item->>'shipped_at','')::date else shipped_at end,
      credit_reference_amount=case when r.item->'proposed_changes'?'credit' then nullif(r.item->>'credit','')::numeric else credit_reference_amount end,
      split_completed_at=case
        when r.item->'proposed_changes'?'split_completed_at' and (r.item->>'split_completed_at') is not null and sale_type='SPLIT'
          then (r.item->>'split_completed_at')::date
  ...
  loop
    v_client_id:=nullif(r.item->>'resolved_client_id','')::uuid;
    ...
    elsif not exists(select 1 from public.clients where id=v_client_id and organization_id=p_organization_id and deleted_at is null) then
      raise exception 'resolved_client_outside_organization';
    end if;
    v_perfume_id:=nullif(r.item->>'resolved_perfume_id','')::uuid;
    ...
    elsif not exists(select 1 from public.perfumes where id=v_perfume_id and organization_id=p_organization_id) then
      raise exception 'resolved_perfume_outside_organization';
    end if;
    ...
    if exists(select 1 from public.sales s where s.organization_id=p_organization_id and s.deleted_at is null
      ...
      and s.import_batch_id is distinct from v_batch_id)
    ...
    insert into public.sales(
      ...
      nullif(r.item->>'credit','')::numeric,
      case when upper(r.item->>'type')='SPLIT' and (r.item->>'split_completed_at') is not null
        then (r.item->>'split_completed_at')::date else null end,
      false
