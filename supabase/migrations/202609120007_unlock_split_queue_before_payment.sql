...
-- Davi precisa enxergar o que falta separar (SPLIT/APC) antes de marcar a
-- venda como paga, não só depois. 202609120006 fazia a fila do Gabriel
-- (sale_split_status_cards/_perfume_summary/_list) e as ações de marcar
-- separado (set_sale_split_status/_bulk, complete_sale_splits_for_filter)
-- exigirem que a venda já estivesse com pagamento confirmado. Este migration
-- remove essa exigência: a fila passa a listar SPLIT/APC em qualquer status
-- de pagamento, e as ações de marcar separado deixam de bloquear por
-- pagamento pendente. Nenhuma constraint de tabela dependia do status de
-- pagamento (só as funções abaixo), então não há alteração de schema.
...
begin
  ...
  where sale.id=any(p_sale_ids)
    ...
    and sale.deleted_at is null and sale.sale_type in('SPLIT','APC')
  for update;
  ...
  with changed as(
    ...
    where sale.id=any(p_sale_ids) and sale.deleted_at is null
...
begin
  ...
  with eligible as(
    ...
    where sale.organization_id in(select public.current_user_org_ids())
      and public.has_org_permission(sale.organization_id,'sales.edit')
      and sale.sale_type in('SPLIT','APC') and sale.deleted_at is null
      and ((sale.sale_type='SPLIT' and sale.split_status='not_split') or (sale.sale_type='APC' and sale.apc_separation_status='not_split'))
...
returns jsonb language sql stable security definer set search_path=public as $$
  with eligible as(
    ...
    where s.organization_id in(select public.current_user_org_ids())
      ...
      and s.sale_type in('SPLIT','APC') and s.deleted_at is null
...
drop index if exists public.sales_org_apc_separation_status_idx;
create index sales_org_apc_separation_status_idx on public.sales (organization_id,apc_separation_status)
  where deleted_at is null and sale_type='APC';
