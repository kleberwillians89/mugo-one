...
-- ==========================================================================
-- IMPORTAÇÃO INCREMENTAL — suporte a DATA DO SPLIT (sales.split_completed_at)
-- --------------------------------------------------------------------------
-- A migration histórica 202608130002 NÃO é editada. Esta republica a RPC
...
--
-- Adição segura:
--   - NEW  (new_safe): grava split_completed_at quando informado e a venda é SPLIT;
--   - CHANGED (existing_changed): atualiza split_completed_at da MESMA sale.id
--     quando informado e a venda é SPLIT — nunca cria venda nova, nunca
--     apaga uma data existente (valor nulo = no-op).
-- ==========================================================================
  ...
  loop
    ...
    update public.sales set
      ...
      split_completed_at=case
        when (r.item->>'split_completed_at') is not null and sale_type='SPLIT'
