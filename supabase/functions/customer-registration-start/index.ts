...
Deno.serve(async(req)=>{
  ...
  const {data:generated,error}=await admin.auth.admin.generateLink({type:'invite',email,options:{redirectTo:`${Deno.env.get('RUAH_PORTAL_URL')??'https://crm.ruahparfums.com.br'}/minha-ruah/criar-senha?flow=invite`,data:{full_name:name,phone,ruah_public_registration:true}}})
