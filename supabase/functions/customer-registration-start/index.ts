...
Deno.serve(async(req)=>{
  ...
  try {
    const [addressAllowed,emailAllowed]=await Promise.all([
      publicRateLimit({admin,req,serviceRoleKey:key,scope:'customer-registration-start.ip',limit:20,windowSeconds:3600}),
      publicRateLimit({admin,req,serviceRoleKey:key,scope:'customer-registration-start.email',identifier:email,includeAddress:false,limit:5,windowSeconds:3600}),
    ])
    if(!addressAllowed||!emailAllowed)return json({error:{code:'rate_limit',message:'Muitas solicitações. Aguarde antes de tentar novamente.'}},429,req)
  } catch { return json({error:{code:'service_unavailable',message:'Cadastro temporariamente indisponível.'}},503,req) }
  ...
  if(error||!generated.user||!generated.properties?.action_link){const alreadyExists=/already.*(?:registered|exists)|user.*exists/i.test(`${error?.code??''} ${error?.message??''}`);console.error({event:'auth_link_generation_failed',email:maskedEmail(email),code:alreadyExists?'account_already_exists':'invite_link_failed'});console.error({event:'registration_failed',stage:'auth_link',code:alreadyExists?'account_already_exists':'invite_link_failed'});return alreadyExists?json(generic,200,req):json({error:{code:'invite_link_failed',message:'Não foi possível preparar seu convite agora. Tente novamente em instantes.'}},502,req)}
