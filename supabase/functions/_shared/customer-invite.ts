...
export async function sendInviteEmail(input:{to:string;name:string;actionLink:string;source:InviteSource}):Promise<DeliveryResult> {
  ...
  try {
    const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify({from,to:[input.to],subject:'Seu acesso ao Portal do Cliente está pronto',html:renderCustomerInviteEmail(input),text:renderCustomerInviteText(input)})})
