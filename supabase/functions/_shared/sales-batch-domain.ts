...
const clean=(value:string)=>value.replace(/[▪️🚨📦💳]/gu,'').trim()
...
export function parseDaviSalesBatch(rawText:string):ParsedSalesBatch{
  const raw=String(rawText||'').trim();if(!raw)throw new Error('empty_batch')
  ...
  const prices=new Map<string,number>();for(const match of raw.matchAll(/^\s*(\d+)\s*ml\s*:\s*R\$\s*([\d.,]+)\s*$/gim))prices.set(String(Number(match[1])),money(match[2]))
  const apcPrice=raw.match(/^\s*APC\s*:\s*R\$\s*([\d.,]+)\s*$/im)
  const sales:ParsedBatchSale[]=[]
  for(const line of lines){const match=line.match(/^(APC|\d+\s*ml)\s*:\s*(.+)$/i);if(!match||/^R\$/i.test(match[2])||/\d+\s*ml\s*\+\s*R\$/i.test(match[2]))continue;const type=match[1].toUpperCase()==='APC'?'APC':'SPLIT',volume=type==='APC'?number(apcRule?.[1]||'0'):number(match[1].replace(/ml/i,''));const client=match[2].trim();if(!client||!volume)continue;const amount=type==='APC'?(apcPrice?money(apcPrice[1]):Math.round((volume*number(quote?.[1]||'0')+number(apcRule?.[2]||'0'))*100)/100):(prices.get(String(volume))??Math.round(volume*number(quote?.[1]||'0')*100)/100);sales.push({client_name:client,sale_type:type,volume_ml:volume,amount,raw:line,phone:null,address:null})}
