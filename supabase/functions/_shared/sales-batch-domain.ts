...
export function countSaleCandidateLines(value:string){return normalizeSalesBatchText(value).split('\n').map(clean).filter(line=>parseRemainingAvailableLine(line)===null&&/^(?:APC(?:\s+\d+(?:[.,]\d+)?\s*mls?)?|\d{1,3}\s*mls?)\s*[:\-–—]/i.test(line)&&!/^\s*(?:APC|\d{1,3}\s*mls?)\s*[:\-–—]\s*R\$/i.test(line)).length}
...
export function parseDaviSalesBatch(rawText:string):ParsedSalesBatch{
  ...
  const lines=raw.split(/\r?\n/).map(clean).filter(Boolean),firstLine=lines[0]||'',title=/^(?:APC(?:\s+\d+(?:[.,]\d+)?\s*mls?)?|\d{1,3}\s*mls?)\s*[:\-–—]/i.test(firstLine)?'':firstLine
  ...
  const prices=new Map<string,number>();for(const line of lines){const match=line.match(/^(\d{1,3})\s*mls?\s*[:\-–—]\s*R\$\s*([\d.,]+)\s*$/i);if(match)prices.set(String(Number(match[1])),money(match[2]))}
  ...
  const unpricedBuyerVolumes=lines.flatMap(line=>{const match=line.match(/^(\d{1,3})\s*mls?\s*[:\-–—]\s*(?!R\$)(.+)$/i);return match&&parseRemainingAvailableLine(line)===null?[number(match[1])]:[]}).filter(volume=>!prices.has(String(volume)))
  ...
  for(const line of lines){if(parseRemainingAvailableLine(line)!==null)continue;const match=line.match(/^(APC(?:\s+(\d+(?:[.,]\d+)?)\s*mls?)?|\d{1,3}\s*mls?)\s*[:\-–—]\s*(.+)$/i);if(!match||/^R\$/i.test(match[3])||/\d+\s*mls?\s*\+\s*R\$/i.test(match[3]))continue;const explicitApcVolume=match[2]?number(match[2]):null,numericVolume=/^APC/i.test(match[1])?null:number(match[1].replace(/mls?/i,'')),isConfiguredApc=Boolean(numericVolume&&((apcRule&&Math.abs(numericVolume-number(apcRule[1]))<.001)||(inferredApcVolume&&Math.abs(numericVolume-inferredApcVolume)<.001))),type=/^APC/i.test(match[1])||isConfiguredApc?'APC':'SPLIT',volume=type==='APC'?(explicitApcVolume??number(apcRule?.[1]||String(numericVolume??0))):numericVolume??0;const client=clean(match[3]);if(!client||!volume)continue;const advertisedApcVolume=apcRule?number(apcRule[1]):null,useAdvertisedApcPrice=type==='APC'&&apcPrice&&advertisedApcVolume!==null&&Math.abs(volume-advertisedApcVolume)<.001;const amount=type==='APC'?(useAdvertisedApcPrice?money(apcPrice![1]):Math.round((volume*(quoteValue??0)+apcExtraValue)*100)/100):(prices.get(String(volume))??Math.round((volume*(quoteValue??0)+recrimpingValue)*100)/100);sales.push({client_name:client,sale_type:type,volume_ml:volume,amount,raw:line,phone:null,address:null})}
