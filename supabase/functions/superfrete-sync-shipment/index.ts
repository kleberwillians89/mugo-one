...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const {data:updated,error:updateError}=await ctx.client.from('shipments').update({print_url:trustedPrintUrl||null,label_pdf_url:trustedPrintUrl||null,print_available:probe.available,print_http_status:probe.httpStatus,print_content_type:probe.contentType,print_checked_at:new Date().toISOString(),integration_error:rawPrintUrl&&!probe.available&&printableStatus?'SUPERFRETE_PRINT_UNAVAILABLE':null}).eq('id',shipmentId).eq('organization_id',ctx.organizationId).select('*').single()
