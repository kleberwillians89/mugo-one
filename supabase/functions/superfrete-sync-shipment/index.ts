...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    try{const safeUrl=printUrl?new URL(printUrl):null;printHost=safeUrl?.hostname??null;printPath=safeUrl?.pathname??null}catch{/* URL externa inválida; nunca registrar o valor bruto */}
    console.log(JSON.stringify({stage:'print_health',shipment_id:shipmentId,order_id_suffix:String(shipment.superfrete_order_id).slice(-6),external_status:state.status??null,has_tracking:Boolean(state.tracking),has_print_url:Boolean(printUrl),print_url_host:printHost,print_url_path:printPath,print_http_status:printStatus,print_content_type:printType,print_available:printAvailable}))
