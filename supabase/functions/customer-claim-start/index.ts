...
Deno.serve(async (req) => {
  ...
  const { data: client } = await admin.from('clients').select('id,organization_id')
  ...
  // Idempotência: já ativa, ou convite disparado há menos de 2 minutos —
  // não reenvia (evita ruído/abuso), mas a resposta continua a mesma.
  const recentlyInvited = account.last_invited_at && Date.parse(account.last_invited_at) > Date.now() - 120_000
  if (account.status === 'active' || (account.auth_user_id && recentlyInvited)) return json(generic, 200, req)
