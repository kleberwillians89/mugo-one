...
| ~~`INTERNAL_DOMAIN = 'acesso.ruahparfums.com.br'`~~ **[RESOLVIDO — ver Sprint de Limpeza, docs/ACTIVE_LEGACY_COLLECTIONS_AUDIT.md]** | `supabase/functions/admin-create-user/index.ts:9` | Já corrigido em sprint intermediária: o código atual usa `INTERNAL_DOMAIN = 'acesso.mugo.one'` com comentário explícito "nenhum tenant específico é hardcoded aqui" — linha mantida aqui só como histórico da decisão original | — |
