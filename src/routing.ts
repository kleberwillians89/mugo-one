import {
  Boxes, CircleDollarSign, FileSpreadsheet, FileText, Home, Import, ListChecks, Lightbulb, Package, Radar, Settings, ShoppingBag, Sparkles, Truck, UserRound, UsersRound,
} from 'lucide-react'
...
export type Page = 'Visão Geral'|'Tarefas'|'Clientes'|'Vendas'|'Produtos'|'Planilha'|'Cobranças'|'Entregas'|'Estoque'|'Relatórios'|'Importação'|'IA'|'Insights'|'Radar'|'Interessados'|'Configurações'

export const navigation: { label: Page; icon: typeof Home }[] = [
  { label: 'Visão Geral', icon: Home }, { label: 'Tarefas', icon: ListChecks },
...
export const routes:Record<Page,string>={'Visão Geral':'/','Tarefas':'/tarefas','Clientes':'/clientes','Vendas':'/vendas','Produtos':'/produtos','Planilha':'/planilha','Cobranças':'/cobrancas','Entregas':'/entregas','Estoque':'/estoque','Relatórios':'/relatorios','Importação':'/importacao','IA':'/ia','Insights':'/insights','Radar':'/radar','Interessados':'/interessados','Configurações':'/configuracoes'}
...
export const pagePermission:Record<Page,string[]>={
  'Visão Geral':['dashboard.view'],'Tarefas':['tasks.view'],
...
export const pageFeature:Partial<Record<Page,FeatureCode>>={
  'Tarefas':'tasks','Estoque':'inventory','Entregas':'shipping','Radar':'radar','Interessados':'waitlist',
}
