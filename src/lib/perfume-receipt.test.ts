...
describe('recebimento canônico por RUAH-P',()=>{
  it('bip monta preview read only antes de qualquer confirmação',()=>{const preview=sql.slice(sql.indexOf('perfume_receipt_preview'),sql.indexOf('perfume_receipt_confirm'));expect(preview).not.toMatch(/\b(insert|update|delete)\b/i);expect(station.indexOf('previewPerfumeReceipt')).toBeLessThan(station.indexOf('CONFIRMAR RECEBIMENTO'))})
