...
describe('Minha RUAH first access',()=>{
  ...
  it('keeps recovery separate from first access',()=>{expect(root).toContain('/minha-ruah/redefinir-senha`');expect(root).toContain("path === '/minha-ruah/redefinir-senha'");expect(root).toContain("callbackFlow!=='recovery'");expect(root).toContain('<ActivateCompletePage recovery />')})
