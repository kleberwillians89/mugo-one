...
describe('Portal do Cliente first access',()=>{
  it('routes every current invite through the canonical public URL configuration',()=>{for(const source of [registration,administrativeInvite,claim])expect(source).toContain('firstAccessRedirectUrl()');expect(publicUrl).toContain("DEPLOYED_PUBLIC_APP_URL='https://crm.ruahparfums.com.br'");expect(publicUrl).toContain("Deno.env.get('PUBLIC_APP_URL')")})
