...
describe('Minha RUAH recovery e-mail handoff',()=>{
  ...
  it('uses the canonical recovery destination',()=>{expect(portal).toContain("redirectTo:`${location.origin}/minha-ruah/redefinir-senha`");expect(portal).toContain("path === '/minha-ruah/redefinir-senha'")})
  ...
  it('provides a premium Portuguese Supabase Reset Password template',()=>{expect(template).toContain('{{ .ConfirmationURL }}');expect(template).toContain('REDEFINIR MINHA SENHA');expect(template).toContain('Redefina sua senha');expect(template).toContain('RUAH PARFUMS');expect(template).not.toMatch(/Reset your password|Supabase Auth|powered by Supabase/i)})
