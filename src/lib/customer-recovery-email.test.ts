...
describe('Minha RUAH recovery e-mail handoff',()=>{
  ...
  it('keeps first access and recovery as separate routes',()=>{expect(portal).toContain("path === '/minha-ruah/criar-senha'");expect(portal).toContain("path === '/minha-ruah/redefinir-senha'");expect(portal).toContain("callbackFlow!=='invite'");expect(portal).not.toContain("callbackFlow!=='recovery'")})
  it('accepts a valid recovery session after Supabase removes callback parameters',()=>{const complete=portal.slice(portal.indexOf('function ActivateCompletePage'),portal.indexOf('function RegistrationPage'));expect(complete).toContain('const invalidCallback=!recovery&&');expect(complete).toContain('auth.getSession()');expect(complete).toContain('auth.getUser()');expect(complete).not.toContain('finalizeCustomerIdentity');expect(complete).toContain("recovery?'Defina sua nova senha'")})
