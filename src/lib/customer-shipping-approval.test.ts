...
describe('team-owned shipping approval',()=>{
  ...
  it('shows approval to staff and only status tracking to the customer',()=>{expect(admin).toContain('APROVAR FRETE PELO TIME');expect(admin).toContain('✓ APROVADO PELO TIME');expect(portal).toContain('EM APROVAÇÃO PELA RUAH');expect(portal).toContain('Nenhuma ação é necessária');expect(portal).not.toContain('confirmCustomerShipment')})
})
