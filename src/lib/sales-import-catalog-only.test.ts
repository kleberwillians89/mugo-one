...
describe('novas vendas resolvem catálogo sem fabricar estoque',()=>{
  ...
  it('frontend bloqueia por perfume_id, não por existência de estoque',()=>{expect(ui).toContain('if(!group.perfume_id)');expect(ui).toContain("if(!preview.perfume_id)");expect(records).toContain("if(!preview.perfume_id)throw new Error('Resolva o perfume de catálogo.')")})
