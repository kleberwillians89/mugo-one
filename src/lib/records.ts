...
export async function parseSalesBatch(text:string,saleDate:string){const {data,error}=await supabase!.functions.invoke('parse-sales-batch',{body:{text,sale_date:saleDate}});if(error){let message='Não foi possível analisar a lista.';try{message=(await (error as {context?:Response}).context?.clone().json())?.error?.message??message}catch{/* resposta não JSON */}throw new Error(message)}return data.data as AiSalesBatchPreview}
