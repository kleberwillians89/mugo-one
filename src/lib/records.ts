...
export type CommercialSale = {
  ...
  legacy_shipping_confirmation?:'pending'|'sent'|'not_sent'|null;legacy_shipping_confirmed_at?:string|null;legacy_shipping_confirmed_by?:string|null
...
export async function fetchSale360(saleId:string){
  const {organizationId}=await authenticatedOrganization()
  const {data,error}=await supabase!.from('sales').select('id,updated_at,client_id,perfume_id,sale_date,amount,payment_status,payment_method,paid_at,original_client,perfume_name_raw,bottle_identifier,sale_type,volume_ml,shipping_deadline_raw,shipping_deadline_date,shipping_availability_text,shipping_availability_kind,shipping_available_date,shipping_lead_business_days,shipping_availability_confirmed_at,shipping_operational_status,shipped_at,legacy_shipping_confirmation,legacy_shipping_confirmed_at,legacy_shipping_confirmed_by,notes,source,credit_reference_amount,inventory_allocation_eligible,clients(name,phone,whatsapp_phone,email,cpf,cnpj,postal_code,address_line,address_number,complement,district,city,state)').eq('organization_id',organizationId).eq('id',saleId).is('deleted_at',null).single()
  if(error)throw new Error(error.message)
...
export async function setLegacyShippingConfirmation(saleId:string,confirmation:'pending'|'sent'|'not_sent',expectedUpdatedAt:string,note?:string) {
  await authenticatedOrganization()
  const {data,error}=await supabase!.rpc('set_legacy_shipping_confirmation',{p_sale_id:saleId,p_confirmation:confirmation,p_expected_updated_at:expectedUpdatedAt,p_note:note?.trim()||null})
  if(error){
    const message=String(error.message)
  ...
  return data as{sale_id:string;confirmation:'pending'|'sent'|'not_sent';confirmed_at:string|null;confirmed_by:string|null;updated_at:string;changed:boolean}
}
