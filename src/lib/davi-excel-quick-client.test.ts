...
describe('cadastro rápido de cliente no Davi Excel',()=>{
  it('oferece CTA no topo somente sem correspondência exata normalizada',()=>{expect(grid).toContain('CADASTRAR “${query}”');expect(grid).toContain('createActionPosition="top"');expect(grid).toContain('normalizeClient(query)===normalizeClient(option.label)');expect(combo).toContain('shouldOfferCreate(query.trim(),options)');expect(combo).toContain("createActionPosition==='top'")})
