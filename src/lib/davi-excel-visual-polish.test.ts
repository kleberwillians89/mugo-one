...
const page=readFileSync('src/legacy/spreadsheet/DaviExcelPage.tsx','utf8'),drafts=readFileSync('src/legacy/spreadsheet/DaviExcelNewRows.tsx','utf8'),css=readFileSync('src/legacy/spreadsheet/DaviExcelPolish.css','utf8')+readFileSync('src/legacy/spreadsheet/DaviExcelNewRows.css','utf8')
describe('polimento visual do Davi Excel',()=>{
 ...
 it('usa a altura útil e maximiza o modo expandido, mantendo o cabeçalho visível',()=>{expect(css).toContain('height:calc(100dvh - 190px)');expect(css).toContain('.davi-excel.expanded .davi-grid{height:calc(100dvh - 118px)');expect(css).not.toContain('.davi-excel.expanded .davi-title{display:none}')})
})
