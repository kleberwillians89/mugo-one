...
describe('estoque visual e reparo do perfume da venda',()=>{
  it('troca a tabela por cards pesquisáveis, compactos e responsivos',()=>{expect(inventory).toContain('inventory-card-grid');expect(inventory).toContain('Buscar perfume no estoque');for(const label of['DISPONÍVEL PARA VENDA','Físico','Reservado','Em preparo','Em envio','Custo/ML'])expect(inventory).toContain(label);expect(inventoryCss).toContain('repeat(auto-fill,minmax(245px,1fr))');expect(inventoryCss).toContain('min-height:31px')})
  ...
  it('deixa o saldo como leitura das vendas, sem ajuste manual concorrente',()=>{expect(inventory).toContain('Saldo automático calculado pelas vendas validadas');expect(inventory).toContain('Atualizar vendas');expect(inventory).not.toContain('Ajustar saldo')})
