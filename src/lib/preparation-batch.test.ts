...
describe('um código por perfume e fracionamento em lote',()=>{
 ...
 it('scan apenas identifica e confirmação é separada/idempotente',()=>{expect(sql).toContain("status='identified'");expect(sql).toContain("if b.status='confirmed' then return jsonb_build_object");expect(page).toContain('CONFIRMAR FRACIONAMENTO?')})
