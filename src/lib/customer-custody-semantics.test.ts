...
describe('customer custody integration semantics',()=>{
  ...
  it('renders all four customer-facing buckets and preserves committed perfumes',()=>{for(const copy of ['Disponível para envio','Aguardando sua aprovação','Em preparação','Em transporte','Total meu na RUAH','VER ENVIO / APROVAR','APROVAR ENVIO'])expect(portal).toContain(copy)})
