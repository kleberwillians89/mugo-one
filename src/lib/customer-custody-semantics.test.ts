...
describe('customer custody integration semantics',()=>{
  ...
  it('renders all four customer-facing buckets and preserves committed perfumes',()=>{for(const copy of ['Disponível para envio','Em aprovação pela RUAH','Em preparação','Em transporte','sob os cuidados da RUAH','Em solicitação:'])expect(portal).toContain(copy);expect(portal).not.toContain('APROVAR ENVIO')})
  it('contains the administrative team-approval action',()=>{expect(admin).toContain('APROVAR FRETE PELO TIME');expect(admin).toContain('approveShipmentForLabel')})
})
