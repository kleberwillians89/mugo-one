...
describe('customer custody integration semantics',()=>{
  ...
  it('renders all four customer-facing buckets and preserves committed perfumes',()=>{for(const copy of ['Disponível para envio','Em aprovação','Em preparação','Em transporte','sob nossos cuidados','Em solicitação:'])expect(portal).toContain(copy);expect(portal).not.toContain('APROVAR ENVIO')})
