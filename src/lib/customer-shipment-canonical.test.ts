...
describe('canonical customer shipment approval',()=>{
  ...
  it('merges canonical shipments with only unconverted requests and deduplicates converted ones',()=>{expect(data).toContain("supabase!.rpc('customer_shipments_list')");expect(data).toContain('const shipmentIds=new Set');expect(data).toContain('!shipmentIds.has(item.converted_shipment_id)');expect(data).not.toContain("supabase!.rpc('customer_shipment_confirm'")})
  it('keeps approval read-only in the portal and exposes it to the team',()=>{expect(portal).toContain('Nenhuma ação é necessária');expect(portal).not.toContain('confirmCustomerShipment');expect(admin).toContain('approveShipmentForLabel');expect(teamApproval).toContain('team_shipping_approved')})
  ...
  it('does not erase custody when a shipment RPC is temporarily unavailable',()=>{expect(portal).toContain('Promise.allSettled([fetchCustody(), fetchMyRequests(), fetchMyTickets()])');expect(portal).toContain("if (c.status === 'fulfilled') setCustody(c.value)")})
