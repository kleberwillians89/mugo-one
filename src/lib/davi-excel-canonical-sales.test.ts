...
describe('Davi Excel como entrada canônica de vendas',()=>{
 ...
 it('edição de sale_id usa update, nunca insert, e Backspace não exclui',()=>{expect(page).toContain('updateDaviExcelSale(row.id');expect(records).toContain("rpc('davi_excel_update_sale'");expect(page).not.toContain('createSale(row');expect(ui).not.toContain("event.key==='Delete'");expect(ui).not.toContain("event.key==='Backspace'")})
