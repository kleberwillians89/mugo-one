...
describe('Davi Excel como entrada canônica de vendas',()=>{
 ...
 it('cada salvamento chama a RPC canônica com frasco e usa o UUID retornado',()=>{expect(records).toContain("rpc('davi_excel_create_sale_with_bottle'");expect(records).toContain('return{id:String(data),alreadyExisted:false}');expect(ui).toContain('saleId:result.id');expect(ui).toContain('setLastSaleId(result.id)');expect(ui).toContain('ABRIR VENDA 360')})
 ...
 it('edição de sale_id usa update com frasco, nunca insert, e Backspace não exclui',()=>{expect(page).toContain('updateDaviExcelSale(row.id');expect(records).toContain("rpc('davi_excel_update_sale_with_bottle'");expect(page).not.toContain('createSale(row');expect(ui).not.toContain("event.key==='Delete'");expect(ui).not.toContain("event.key==='Backspace'")})
