...
describe('Davi Excel canônico',()=>{
 ...
 it('crédito e logística usam contrato seguro, bloqueando data controlada por shipment',()=>{expect(safeUpdate).toContain("'credit_reference_amount'");expect(safeUpdate).toContain("'shipped_at'");expect(safeUpdate).toContain('shipment_controls_shipping_date');expect(page).toContain('Data controlada pelo shipment')})
 it('edição exige sales.edit, mantém erro editável e exporta sem UUID',()=>{expect(safeUpdate).toContain("has_org_permission(before_row.organization_id,'sales.edit')");expect(page).toContain('setError(message)');expect(page).toContain("exportCsv('davi-excel.csv'");expect(page).not.toContain("'UUID'")})
