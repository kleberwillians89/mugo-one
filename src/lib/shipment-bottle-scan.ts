 ...
 * never a raw error code (briefing: "⚠ FRASCO DIFERENTE", not a technical
 * message). Handles both physical objects the RPC can resolve: a source
 * bottle or a pre-fractionated split unit (Priority 0B) — never confused
 * with each other in the message either.
 *
 * Três tons, não dois (briefing seção 10/12 — VERDE/VERMELHO/AMARELO):
 * 'error' é para um objeto ERRADO ou indisponível (nunca deveria ser
 * separado daqui); 'warning' é para o objeto CERTO mas com quantidade
 * insuficiente (o frasco é o certo, só não tem ml suficiente agora — não é
 * o mesmo tipo de bloqueio visual que "frasco de outro perfume").
 */
export function describeBottleScanResult(result: BottleScanResult): { tone: 'success'|'warning'|'error'; message: string } {
  ...
  switch (result.reason) {
    ...
    case 'insufficient_ml': return { tone: 'warning', message: `⚠ VOLUME INSUFICIENTE — este frasco só tem ${formatMl(result.available_ml ?? 0)}, o pedido precisa de ${formatMl(result.needed_ml ?? 0)}.` }
    ...
    case 'split_quantity_mismatch': return { tone: 'warning', message: `⚠ SPLIT DIFERENTE — ${result.split_code ?? 'este split'} tem ${formatMl(result.split_ml ?? 0)}, o pedido precisa de ${formatMl(result.needed_ml ?? 0)}.` }
