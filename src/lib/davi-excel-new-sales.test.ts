...
describe('entrada canônica de vendas no Davi Excel',()=>{
 it('mantém rascunhos locais, múltiplas linhas e campos read only',()=>{expect(ui).toContain('NOVA LINHA');expect(ui).toContain('SALVAR {valid');expect(ui).toContain('davi-readonly');expect(ui).toContain('rascunho')})
 ...
 it('reutiliza createSale com origem, frasco e idempotência próprias',()=>{expect(ui).toContain("source:'davi_excel'");expect(ui).toContain('idempotencyKey:row.key');expect(ui).toContain('bottleNumber:Number(row.bottleNumber)');expect(records).toContain("input.source==='davi_excel'");expect(records).toContain("rpc('davi_excel_create_sale_with_bottle'")})
 ...
 it('linhas locais permanecem na mesma grade acima dos resultados e somente editores veem criação',()=>{expect(page).toContain('canEdit&&<DaviExcelNewRows');expect(page.indexOf('DaviExcelNewRows onCreated')).toBeLessThan(page.indexOf('rows.map'))})
})
