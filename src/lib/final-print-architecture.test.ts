...
const perfumePage=readFileSync('src/legacy/operations/pages/PerfumePrintLabelPage.tsx','utf8')
const perfumeCss=readFileSync('src/legacy/operations/pages/PerfumePrintLabelPage.css','utf8')
...
describe('arquitetura final das impressões RUAH (etiqueta de perfume isolada como legado — ver src/legacy/operations/)',()=>{
  it('A4 (shipment, ainda ativo) e etiqueta 70x30 (perfume, legado) mantêm CSS de página distinto',()=>{expect(routes).toContain("path==='/print/shipment'");expect(routes).not.toContain("path==='/print/perfume'");expect(shipmentPage).toContain('@page { size: A4 portrait; margin: 12mm 14mm; }');expect(perfumePage).toContain('@page { size: 70mm 30mm; margin: 0; }');expect(perfumeCss).toContain('width: 70mm');expect(perfumeCss).toContain('height: 30mm')})
