...
describe('arquitetura final das impressões RUAH',()=>{
  it('isola A4 e etiqueta 70x30 em rotas distintas',()=>{expect(routes).toContain("path==='/print/shipment'");expect(routes).toContain("path==='/print/perfume'");expect(shipmentPage).toContain('@page { size: A4; margin: 12mm; }');expect(perfumePage).toContain('@page { size: 70mm 30mm; margin: 0; }');expect(perfumeCss).toContain('width: 70mm');expect(perfumeCss).toContain('height: 30mm')})
  ...
  it('aceita apenas identidade canônica RUAH-P e usa o mesmo payload nos três elementos',()=>{expect(perfumePage).toContain("/^RUAH-P\\d{6}$/.test(code)");expect(perfumePage).toContain('<BarcodeImage value={code}');expect(perfumePage).toContain('<code>{code}</code>');expect(perfumePage).toContain('<QrCodeImage value={code}');expect(printHelper).toContain('codes:perfume.operational_code')})
