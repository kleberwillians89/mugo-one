...
describe('arquitetura final das impressões RUAH',()=>{
  it('isola A4 e etiqueta 70x30 em rotas distintas',()=>{expect(routes).toContain("path==='/print/shipment'");expect(routes).toContain("path==='/print/perfume'");expect(shipmentPage).toContain('@page { size: A4 portrait; margin: 12mm 14mm; }');expect(perfumePage).toContain('@page { size: 70mm 30mm; margin: 0; }');expect(perfumeCss).toContain('width: 70mm');expect(perfumeCss).toContain('height: 30mm')})
