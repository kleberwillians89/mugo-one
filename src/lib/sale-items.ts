 ...
 * no fluxo ativo, o createSale() antigo (perfume-only), que continua
 * existindo só para o Davi Excel legado.
 */
...
export type SaleItemsSummary = { count: number; firstDescription: string }
...
export async function fetchSaleItemsSummaries(saleIds: string[]): Promise<Map<string, SaleItemsSummary>> {
  ...
  const { data, error } = await supabase.from('sale_items').select('sale_id,description,position').in('sale_id', saleIds).order('position')
  if (error) throw new Error(error.message)
  for (const row of (data ?? []) as { sale_id: string; description: string; position: number }[]) {
    const current = map.get(row.sale_id)
    if (!current) map.set(row.sale_id, { count: 1, firstDescription: row.description })
