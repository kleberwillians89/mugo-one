...
describe('transporte CORS da inteligência',()=>{
  it('autoriza a origem pública central e todos os headers do cliente Supabase',()=>{expect(security).toContain('allowedPublicOrigins()');expect(security).toContain('authorization, apikey, content-type, x-client-info')})
...
describe('impressão SuperFrete sem nova compra',()=>{
  ...
  it('não responde sucesso quando a saúde de impressão não foi persistida',()=>{expect(sync).toContain("code:'SUPERFRETE_PRINT_PERSIST_ERROR'");expect(sync).toContain("ctx.client.rpc('shipment_set_superfrete_print_health'");expect(sync).toContain('data:updated')})
})
