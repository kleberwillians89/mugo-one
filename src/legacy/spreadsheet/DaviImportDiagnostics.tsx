...
export function DaviImportDiagnostics({onApplied=async()=>{}}:{onApplied?:()=>void|Promise<void>}){
 ...
 const applySafe=async()=>{if(!file||!report||!safe.length)return;setApplying(true);setError('');try{const result=await applySafeDaviDiagnostic(report);await onApplied();const refreshed=await analyzeDaviFile(file);setReport(refreshed);toast.push(`${result.applied.toLocaleString('pt-BR')} alterações aplicadas com sucesso`,{tone:'success',duration:6500})}catch(reason){const raw=reason instanceof Error?reason.message:'Não foi possível aplicar as alterações.'
  // O código do conflito vem como texto de um jsonb do Postgres (RAISE EXCEPTION
  // 'davi_safe_apply_conflict:%', jsonb::text) — o cast normaliza espaçamento
  // (ex.: "code": "x", com espaço após os dois-pontos), então só o prefixo
  // literal é estável para detecção; nunca tentamos casar o JSON inteiro.
  const isStaleConflict=raw.startsWith('davi_safe_apply_conflict:')
  if(isStaleConflict){console.warn('[davi-safe-apply] lote rejeitado por divergência de estado',raw);setReport(null);setRefreshing(true);try{const refreshed=await analyzeDaviFile(file);setReport(refreshed);setError('');toast.push('A base mudou desde a última análise. O diagnóstico foi atualizado automaticamente. Confira os novos resultados antes de aplicar.',{tone:'info',duration:7000})}catch{setError('A base mudou. Analise novamente a planilha antes de aplicar.')}finally{setRefreshing(false)}}else{setError(raw);toast.push(raw,{tone:'error',duration:7000})}}finally{setApplying(false)}}
 ...
 return <section className={`davi-diagnostic ${diagnosticOpen?'is-open':''}`} aria-label="Diagnóstico operacional do Davi Excel">
  ...
  {refreshing&&<p className="davi-diagnostic-refreshing"><LoaderCircle className="spin"/>ATUALIZANDO DIAGNÓSTICO...</p>}
  {report&&<div className="davi-diagnostic-results">
   ...
   <div className="davi-diagnostic-groups">
    ...
    <footer className="davi-diagnostic-actions"><div><strong>{safe.length} item(ns) seguro(s)</strong><span>{incomplete.length?`${incomplete.length} linha${incomplete.length===1?'':'s'} requer nova análise. `:''}Duplicidades, conflitos, inválidas e falhas de estoque nunca entram no apply.</span></div><button type="button" disabled={!safe.length||applying||refreshing} onClick={()=>void applySafe()} title={safe.length?'Revalida e aplica atomicamente somente as linhas seguras.':'Nenhuma alteração segura para aplicar.'}>{applying?<><LoaderCircle className="spin"/>APLICANDO...</>:'APLICAR ALTERAÇÕES SEGURAS'}</button></footer>
  </div>}</details>
 </section>
}
