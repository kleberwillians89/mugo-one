...
export function ClientsPage({period,setPeriod}:{period:PeriodValue;setPeriod:(value:PeriodValue)=>void}) {
  ...
  return <div className="page clients-page">{modal&&<ClientModal close={()=>setModal(false)}/>}
    <PageHeader eyebrow="PRIVATE CLIENT SERVICE" title="Clientes" description="Relacionamento, recorrência e histórico no período." actions={<><PeriodFilter value={period} onApply={setPeriod} allowHistorical/><SecondaryButton onClick={()=>{history.pushState({},'','/clientes/acessos-minha-ruah');dispatchEvent(new PopStateEvent('popstate'))}}>Acessos em análise</SecondaryButton><PrimaryButton icon={<Plus size={16}/>} onClick={()=>setModal(true)}>Adicionar cliente</PrimaryButton></>}/>
