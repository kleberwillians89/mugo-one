...
export function Intelligence({period,setPeriod}:{period:PeriodValue;setPeriod:(value:PeriodValue)=>void}) {
  ...
  return <div className="page intelligence-page"><PageHeader eyebrow="ASSISTENTE EXECUTIVO" title="Mugô One" description="Análise consultiva baseada apenas em agregados reais." actions={<PeriodFilter value={period} onApply={setPeriod}/>}/>
    <div className="intelligence-hero"><div className="hero-spark"><Sparkles/></div><span>MUGÔ ONE</span><h2>Decisões mais claras começam<br/>com as perguntas certas.</h2><p>Respostas baseadas exclusivamente nos dados autorizados da sua operação.</p></div>
    <div className="chat-box card">
      {!answer&&!loading?<div className="chat-empty"><Bot/><h3>Como posso ajudar hoje?</h3><p>Escolha uma sugestão ou escreva uma pergunta sobre os dados da sua operação.</p></div>:null}
      {loading&&<div className="ai-waiting" aria-live="polite"><LoaderCircle className="spin"/><div><strong>Estamos analisando os dados da sua operação.</strong><p>Isso pode levar alguns segundos.</p><span>{waitingMessage}</span></div></div>}
