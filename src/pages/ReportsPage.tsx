...
export function ReportsPage({period,setPeriod}:{period:PeriodValue;setPeriod:(value:PeriodValue)=>void}) {
  ...
  return <div className="page"><PageHeader eyebrow="INTELIGÊNCIA COMERCIAL" title="Relatórios" description="Resumo financeiro e comparação matemática entre períodos." actions={<><PeriodFilter value={period} onApply={setPeriod}/><SecondaryButton icon={<Scale size={16}/>} onClick={goToMarginReport}>Custo e margem</SecondaryButton><SecondaryButton icon={<Download size={16}/>} onClick={()=>exportCsv('relatorio-ruah.csv',rows)}>Exportar</SecondaryButton></>}/>
