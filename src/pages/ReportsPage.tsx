...
export function ReportsPage({period,setPeriod}:{period:PeriodValue;setPeriod:(value:PeriodValue)=>void}) {
  ...
  return <div className="page"><div className="page-lead"><div><h2>Relatórios</h2><p>Resumo financeiro e comparação matemática entre períodos.</p></div><div className="page-actions"><PeriodFilter value={period} onApply={setPeriod}/><button onClick={goToMarginReport}><Scale size={16}/> Custo e margem</button><button onClick={()=>exportCsv('relatorio-ruah.csv',rows)}><Download/> Exportar</button></div></div>
