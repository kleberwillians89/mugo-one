...
import { countLegacyShipping, legacyShippingState, type LegacyShippingConfirmation } from '../lib/legacy-shipping'
...
export function DeliveriesPage({period,setPeriod}:{period:PeriodValue;setPeriod:(value:PeriodValue)=>void}) {
  ...
  const [confirmationFilter,setConfirmationFilter]=useState<'all'|LegacyShippingConfirmation>('pending'),[savingConfirmation,setSavingConfirmation]=useState<string|null>(null),[notSentSale,setNotSentSale]=useState<CommercialSale|null>(null)
  ...
  const saveConfirmation=async(row:CommercialSale,confirmation:LegacyShippingConfirmation)=>{setSavingConfirmation(row.id);setError('');try{const result=await setLegacyShippingConfirmation(row.id,confirmation,row.updated_at);setRows(current=>current.map(item=>item.id===row.id?{...item,legacy_shipping_confirmation:result.confirmation==='pending'?null:result.confirmation,legacy_shipping_confirmed_at:result.confirmed_at,legacy_shipping_confirmed_by:result.confirmed_by,updated_at:result.updated_at}:item));toast.push(confirmation==='sent'?'Envio confirmado.':confirmation==='not_sent'?'Venda confirmada como não enviada.':'Confirmação voltou para A confirmar.',{tone:'success'})}catch(reason){const message=reason instanceof Error?reason.message:'Não foi possível salvar a confirmação.';setError(message);toast.push(message,{tone:'error'});reload()}finally{setSavingConfirmation(null);setNotSentSale(null)}}
  const chooseConfirmation=(row:CommercialSale,value:LegacyShippingConfirmation)=>{if(value==='not_sent'){setNotSentSale(row);return}void saveConfirmation(row,value)}

  return <div className="page">
    {newShipment&&<NewShipmentModal close={()=>setNewShipment(false)}/>}
    <Modal open={Boolean(notSentSale)} onClose={()=>setNotSentSale(null)} eyebrow="CONFERÊNCIA MANUAL" title="Confirmar como não enviado?">
      <div className="record-form"><p>Esta ação não cancela a venda nem altera estoque ou pagamento.<br/>Ela serve somente para conferência histórica.</p><div className="form-actions"><SecondaryButton onClick={()=>setNotSentSale(null)}>CANCELAR</SecondaryButton><PrimaryButton loading={Boolean(notSentSale&&savingConfirmation===notSentSale.id)} onClick={()=>notSentSale&&void saveConfirmation(notSentSale,'not_sent')}>CONFIRMAR</PrimaryButton></div></div>
    </Modal>
    ...
    {loading?<div className="empty card"><h3>Carregando entregas…</h3></div>:<>{error&&<div className="notice"><AlertTriangle/><span>{error}</span></div>}<div className="card clients-table"><div className="clients-caption"><strong>{integer(visible.length)} registros históricos</strong><span className="live-dot">SUPABASE</span></div>
      <Table
        ...
        columns={[
          ...
          {key:'confirmation',label:'Confirmação',render:(row)=>{const state=legacyShippingState(row.legacy_shipping_confirmation),saving=savingConfirmation===row.id;return <select className={`legacy-confirmation-select legacy-confirmation-select--${state}`} aria-label={`Confirmação manual de ${row.clients?.name??row.original_client??'venda'}`} value={state} disabled={!canEdit||saving} onChange={event=>chooseConfirmation(row,event.target.value as LegacyShippingConfirmation)}>{saving?<option value={state}>SALVANDO...</option>:<><option value="pending">A CONFIRMAR</option><option value="sent">ENVIADO</option><option value="not_sent">NÃO ENVIADO</option></>}</select>}},
