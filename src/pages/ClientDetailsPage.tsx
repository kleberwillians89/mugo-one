...
export function ClientDetailsPage({clientId}:{clientId:string}) {
  ...
  return <div className="page client-360 client-dossier">
    ...
    <div className="card clients-table"><div className="table-wrap"><table><thead><tr><th>Compra</th><th>Perfume</th><th>Prazo previsto</th><th>Data de envio</th><th>Status histórico</th><th>Dias até envio</th><th>Prazo</th><th>Ação</th></tr></thead><tbody>{visibleHistory.map((sale)=>{const days=sale.sale_date&&sale.shipped_at?Math.round((new Date(String(sale.shipped_at)).valueOf()-new Date(String(sale.sale_date)).valueOf())/86400000):null,deadline=sale.shipping_deadline_date?String(sale.shipping_deadline_date):null;return <tr key={`log-${String(sale.id)}`}><td>{sale.sale_date?shortDate(String(sale.sale_date)):'—'}</td><td><strong>{String(sale.perfume_name_raw??'—')}</strong></td><td>{deadline?shortDate(deadline):String(sale.shipping_deadline_raw??'—')}</td><td>{sale.shipped_at?shortDate(String(sale.shipped_at)):'—'}</td><td><LegacyShippingStatusInput key={`log-status-${String(sale.id)}:${String(sale.updated_at)}`} saleId={String(sale.id)} status={sale.legacy_shipping_status as string|null} updatedAt={String(sale.updated_at)} onSaved={applyLegacyStatus}/></td><td>{days===null?'—':`${days} dias`}</td><td>{deadline&&sale.shipped_at?(String(sale.shipped_at)<=deadline?'Dentro do prazo':'Atrasado'):'—'}</td><td><button onClick={()=>setPurchase(sale as Record<string,unknown>)}>Ver compra</button></td></tr>})}</tbody></table>{visibleHistory.length===0&&<div className="inline-empty">Nenhum registro histórico.</div>}</div></div>
    ...
    <div className="card clients-table"><div className="table-wrap"><table><thead><tr><th>Data</th><th>Perfume</th><th>Tipo</th><th>ML</th><th>Valor</th><th>Pagamento</th><th>Status histórico</th><th>Ação</th></tr></thead><tbody>{visibleHistory.map((sale)=>{return <tr key={String(sale.id)}><td>{sale.sale_date?shortDate(String(sale.sale_date)):'—'}</td><td><strong>{String(sale.perfume_name_raw??'—')}</strong></td><td>{String(sale.sale_type??'—')}</td><td>{sale.volume_ml===null?'—':`${Number(sale.volume_ml).toLocaleString('pt-BR')} ML`}</td><td>{brl(Number(sale.amount))}</td><td><span className={`badge ${sale.payment_status}`}>{statusLabel[String(sale.payment_status)]}</span></td><td><LegacyShippingStatusInput key={`history-status-${String(sale.id)}:${String(sale.updated_at)}`} saleId={String(sale.id)} status={sale.legacy_shipping_status as string|null} updatedAt={String(sale.updated_at)} onSaved={applyLegacyStatus}/></td><td><button onClick={()=>setPurchase(sale as Record<string,unknown>)}>Ver compra</button></td></tr>})}</tbody></table></div></div>}
  </div>
}
