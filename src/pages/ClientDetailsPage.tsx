...
export function ClientDetailsPage({clientId}:{clientId:string}) {
  ...
  return <div className="page client-360 client-dossier">
    ...
    <SectionHeader title={`${integer(waitingSorted.length)} produtos com a RUAH`} description="Selecione as compras que devem sair juntas em um único envio." action={
      <PrimaryButton disabled={!selected.length||preparing} loading={preparing} onClick={prepare}>{`Preparar envio (${selected.length})`}</PrimaryButton>
    }/>
