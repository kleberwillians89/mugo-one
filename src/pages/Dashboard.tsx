...
export function Dashboard(){
  ...
  return <div className="page intelligent-home">
    ...
    {loading&&!data?<DashboardSkeleton/>:data&&metrics&&previous&&<>
      <section className="home-summary"><div><div className="home-summary-label"><Sparkles/><span>LEITURA DO PERÍODO</span></div><p>{buildDashboardNarrative(data)}</p></div><small>Atualizado {new Date(data.generated_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',timeZone:data.timezone})}</small></section>
...
function SectionHead({eyebrow,title}:{eyebrow:string;title:string}){return <div className="home-section-head"><span>{eyebrow}</span><h3>{title}</h3></div>}
