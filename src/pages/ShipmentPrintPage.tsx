...
export function ShipmentPrintPage(){
  ...
  const localityLine=[shipment.recipient_district,shipment.recipient_city&&shipment.recipient_state?`${shipment.recipient_city} / ${shipment.recipient_state}`:shipment.recipient_city||shipment.recipient_state].filter(Boolean).join(' · ')
  return <main className="shipment-document-page"><style>{PAGE_STYLE}</style><nav className="shipment-document-actions"><button onClick={()=>history.back()}><ArrowLeft/> VOLTAR</button><button onClick={()=>window.print()}><Printer/> IMPRIMIR NOTA DE ENVIO</button></nav><article className="shipment-document">
    ...
    <section className="shipment-document-client"><h2>Cliente e destino</h2><div>{shipment.clients?.name&&<><small>CLIENTE</small><strong>{shipment.clients.name}</strong></>}<small>DESTINATÁRIO</small><strong>{shipment.recipient_name}</strong>{shipment.recipient_phone&&<span>{shipment.recipient_phone}</span>}</div><address>{addressLine&&<span>{addressLine}</span>}{localityLine&&<span>{localityLine}</span>}{shipment.recipient_postal_code&&<span>CEP {shipment.recipient_postal_code}</span>}</address></section>
    ...
    <section className="shipment-document-summary"><div><h2>Resumo operacional</h2><dl><div><dt>Itens</dt><dd>{items.length}</dd></div><div><dt>Total</dt><dd>{totalMl.toLocaleString('pt-BR')} ml</dd></div>{freight!=null&&<div><dt>Frete</dt><dd>{brl(freight)}</dd></div>}{shipment.carrier&&<div><dt>Transportadora</dt><dd>{shipment.carrier}</dd></div>}{shipment.service&&<div><dt>Serviço</dt><dd>{shipment.service}</dd></div>}{shipment.tracking_code&&<div><dt>Rastreio</dt><dd>{shipment.tracking_code}</dd></div>}</dl></div><div><h2>Conferência final</h2><ul>{['Todos os perfumes conferidos','Embalagem conferida','Etiqueta de transporte conferida','Pronto para postagem'].map(label=><li key={label}><span className="shipment-check-box"/>{label}</li>)}</ul></div></section>
