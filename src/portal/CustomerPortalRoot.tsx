...
function PortalShell({ title, subtitle, children, safeCopy='Seus dados são protegidos' }: { title: string; subtitle: string; children: React.ReactNode;safeCopy?:string }) {
  return <main className="portal-auth">
    <div className="portal-auth-brand"><img src="/mugo-logo.png" alt="Portal do Cliente" /><strong>Portal do Cliente</strong></div>
...
function ActivateStartPage() {
  ...
  return <PortalShell title="Ativar seu acesso" subtitle="Confirme seus dados para ativar sua área exclusiva.">
...
function ActivateCompletePage({recovery=false}:{recovery?:boolean}) {
  ...
  if(step==='success')return <PortalShell title={recovery?'Sua senha foi redefinida com sucesso.':'Senha criada com sucesso.'} subtitle="Seu acesso está protegido e pronto para os próximos logins."><button className="portal-submit" onClick={()=>go('/minha-ruah')}>ENTRAR NO PORTAL</button></PortalShell>
...
function RegistrationPage(){
  ...
  if(sentTo)return <PortalShell title="Link enviado para seu e-mail." subtitle="Enviamos as instruções para você criar sua senha e ativar seu acesso ao Portal do Cliente." safeCopy={`Enviamos um link para: ${sentTo}`}><div className="portal-success-copy"><p>Abra sua caixa de entrada e clique em “CRIAR MINHA SENHA”.</p><p>Não encontrou a mensagem? Confira também Spam, Lixo eletrônico e Promoções.</p></div>{error&&<div className="portal-error">{error}</div>}<button className="portal-submit" disabled={loading||seconds>0} onClick={send}>{loading?<LoaderCircle className="spin"/>:seconds>0?`REENVIAR LINK EM ${seconds}S`:'REENVIAR LINK'}</button><button className="portal-link" onClick={()=>go('/minha-ruah/entrar')}>VOLTAR PARA ENTRAR</button></PortalShell>
  return <PortalShell title="Crie seu acesso." subtitle="Tenha seu relacionamento reunido em um único espaço privado."><form className="portal-form" onSubmit={submit}>
...
export function CustomerPortalRoot() {
  ...
  if(review)return <PortalShell title="Estamos finalizando seu acesso." subtitle="Para proteger seus dados, precisamos confirmar algumas informações antes de vincular seu histórico. A nossa equipe foi avisada."><button className="portal-submit" onClick={()=>supabase!.auth.signOut().then(()=>go('/minha-ruah/entrar'))}>Voltar</button></PortalShell>
