...
const shipmentStatusLabel: Record<string, string> = {
  draft: 'Em preparação', requested: 'Em preparação', awaiting_customer_approval: 'Em aprovação',
  customer_approved: 'Frete aprovado', label_pending: 'Emitindo etiqueta', label_released: 'Pronto para postar',
...
function ShippingSnapshotSummary({groups,requests,onRequest,onShipments}:{groups:ReturnType<typeof groupCustomerCustody>;requests:ShipmentRequest[];onRequest:()=>void;onShipments:()=>void}){
 ...
 return <section className="portal-shipping-snapshot">
  {active&&<div className="portal-card"><span>ENVIO EM ANDAMENTO</span><strong>{active.items?.map(item=>`${item.perfume} — ${item.quantity_ml} ml`).join(' · ')||'Solicitação em andamento'}</strong><p>{shipmentStatusLabel[active.shipment_status??'requested']??'Aguardando andamento'}</p><button onClick={onShipments}>ACOMPANHAR ENVIO</button></div>}
...
function HomePage({ custody, requests, onRequest, greeting, onPerfumes, onShipments, onHelp }: { custody: CustodyItem[]; requests: ShipmentRequest[]; onRequest: () => void;greeting:string;onPerfumes:()=>void;onShipments:()=>void;onHelp:()=>void }) {
  ...
  return <div className="portal-page portal-home">
    <section className="portal-welcome"><span>BEM-VINDA À SUA ÁREA PRIVADA</span><h1>Olá{greeting?`, ${greeting}`:''}.</h1><p>Aqui está a sua história com a gente, com a clareza e o cuidado que ela merece.</p></section>
    <section className="portal-collection"><span>MEU ACERVO</span><strong>{groups.length} {groups.length===1?'perfume':'perfumes'} guardados</strong><p>{summary.physicalMl} ml sob nossos cuidados</p><button onClick={onPerfumes}>Ver meus perfumes</button></section>
    <h2>Seu acervo agora</h2>
    <div className="portal-summary"><div><strong>{summary.availableMl} ml</strong><span>Disponível para envio</span></div><div><strong>{summary.awaitingApprovalMl} ml</strong><span>Em aprovação</span></div><div><strong>{summary.preparingMl} ml</strong><span>Em preparação</span></div><div><strong>{summary.inTransitMl} ml</strong><span>Em transporte</span></div></div>
    <div className="portal-home-links"><button onClick={onShipments}><strong>Meus envios</strong><span>Acompanhe cada etapa</span></button><button onClick={onHelp}><strong>Preciso de ajuda</strong><span>Fale diretamente conosco</span></button></div>
    <ShippingSnapshotSummary groups={groups} requests={requests} onRequest={onRequest} onShipments={onShipments}/>
    <h2>Meus perfumes</h2>
    {groups.length === 0 && <p className="portal-empty">Você ainda não tem perfumes guardados.</p>}
    {groups.map((g) => {const activeRequest=g.active_request?.shipment_status==='awaiting_customer_approval'?g.active_request:undefined,forecast=g.allocations.find(item=>item.shipping_requestable===false);return <div className="portal-card" key={g.perfume_id}>
      <strong>{g.perfume_name}</strong><span>{g.total_ml} ml guardados · {g.available_ml} ml disponíveis</span>
      {g.open_requested_ml>0&&<span>{g.open_requested_ml} ml em solicitação</span>}
      {g.pending_availability_ml>0&&<span>{g.pending_availability_ml} ml aguardando preparação{forecast?.shipping_available_date?` · previsão ${new Date(`${forecast.shipping_available_date}T12:00:00`).toLocaleDateString('pt-BR')}`:''}.</span>}
      {activeRequest&&<span>EM APROVAÇÃO PELA NOSSA EQUIPE</span>}
    </div>})}
  </div>
}
...
function ShipmentsPage({ requests, reload, onOpenHistory }: { requests: ShipmentRequest[]; reload: () => void; onOpenHistory: () => void }) {
  ...
  return <div className="portal-page">
    ...
    {[...requests].sort((a,b)=>String(b.requested_at).localeCompare(String(a.requested_at))).map((r) => <div className="portal-card portal-request-card" key={r.request_id}>
      ...
      {r.awaiting_approval && <div className="portal-quote"><div><strong>EM APROVAÇÃO</strong><span>Frete<br/>{[r.carrier,r.service].filter(Boolean).join(' · ')} · {r.shipping_price != null ? brl(r.shipping_price) : '—'}</span><small>Nenhuma ação é necessária. Nossa equipe fará a aprovação.</small></div></div>}
      {r.shipment_status === 'customer_approved' && <div className="portal-quote portal-quote-approved"><div><strong>✓ FRETE APROVADO</strong><span>A equipe seguirá com a preparação.</span></div></div>}
...
export function CustomerPortalApp({ path, navigate, onSignOut }: { path: string; navigate: (p: string) => void; onSignOut: () => void }) {
  ...
  const [onboarding,setOnboarding]=useState(()=>localStorage.getItem('portal_onboarding_done')!=='true')
  ...
  return <div className="portal-app">
    {onboarding&&<div className="portal-onboarding" role="dialog" aria-modal="true" aria-label="Conheça o Portal do Cliente"><section><span>PORTAL DO CLIENTE</span><h2>Bem-vinda ao seu espaço.</h2><div><article><b>01</b><strong>Seu acervo</strong><p>Veja os perfumes que estão sob sua custódia.</p></article><article><b>02</b><strong>Seus envios</strong><p>Solicite e acompanhe seus perfumes.</p></article><article><b>03</b><strong>Atendimento</strong><p>Fale diretamente com nossa equipe.</p></article></div><button className="portal-submit" onClick={()=>{localStorage.setItem('portal_onboarding_done','true');setOnboarding(false)}}>Conhecer meu espaço</button><button className="portal-link" onClick={()=>{localStorage.setItem('portal_onboarding_done','true');setOnboarding(false)}}>Pular apresentação</button></section></div>}
    <header className="portal-header"><strong>Portal do Cliente</strong>{greeting && <em>Olá, {greeting}</em>}</header>
    ...
    <nav className="portal-bottom-nav" aria-label="Navegação do Portal do Cliente">{tabs.map(({ id, label, icon: Icon }) => <button key={id} aria-current={!showHistory&&tab===id?'page':undefined} className={!showHistory&&tab === id ? 'active' : ''} onClick={() => {if(showHistory)navigate('/minha-ruah');setTab(id)}}><Icon size={20} /><span>{label}</span></button>)}</nav>
  </div>
}
