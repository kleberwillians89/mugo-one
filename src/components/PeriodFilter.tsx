...
import { operationalPeriod, operationalStartDate } from '../lib/operational-sales'

const localIso = (date:Date) => format(date,'yyyy-MM-dd')

export function PeriodFilter({value,onApply,allowHistorical=false}:{value:PeriodValue;onApply:(period:PeriodValue)=>void;allowHistorical?:boolean}) {
  const operationalStart=useOperationalSalesStartDate()
  const effectiveValue=allowHistorical?value:operationalPeriod(value,operationalStart)
  const [open,setOpen]=useState(false)
  const [draft,setDraft]=useState(effectiveValue)
  const [startText,setStartText]=useState(isoToBrazilian(effectiveValue.start))
  const [endText,setEndText]=useState(isoToBrazilian(effectiveValue.end))
  ...
  const commitText=()=>{const start=brazilianToIso(startText),end=brazilianToIso(endText);if(!start||!end)return setError('Informe as duas datas no formato DD/MM/AAAA.');if(end<start)return setError('A data final não pode ser anterior à inicial.');if(!allowHistorical&&end<operationalStartDate(operationalStart))return setError('Esse período é anterior ao início da operação atual.');setDraft({start,end,label:'Período personalizado'});setError('')}
  const selected:DateRange={from:parse(draft.start,'yyyy-MM-dd',new Date()),to:parse(draft.end,'yyyy-MM-dd',new Date())}
  const apply=()=>{commitText();const start=brazilianToIso(startText),end=brazilianToIso(endText);if(!start||!end||end<start||(!allowHistorical&&end<operationalStartDate(operationalStart)))return;const next={...draft,start,end,label:draft.label==='Todo o período'?'Todo o período':`${isoToBrazilian(start)} a ${isoToBrazilian(end)}`};onApply(allowHistorical?next:operationalPeriod(next,operationalStart));setOpen(false)}
  return <div className="period-filter" ref={root}>
    <button type="button" className="date-filter" aria-haspopup="dialog" aria-expanded={open} onClick={()=>{if(!open){setDraft(effectiveValue);setStartText(isoToBrazilian(effectiveValue.start));setEndText(isoToBrazilian(effectiveValue.end))}setOpen(!open)}}><CalendarDays size={17}/><span>{effectiveValue.label}</span><ChevronDown size={16}/></button>
    {open&&<div className="period-popover" role="dialog" aria-modal={compact} aria-label="Selecionar período">
      ...
      <div className="period-panel-body">
        <div className="period-presets">{([
          ...
          ['month','Este mês','Este mês'],...(allowHistorical?[["previous_month",'Mês anterior','Mês anterior'] as [Preset,string,string]]:[]),['quarter','Este trimestre','Trimestre'],
          ['year','Este ano','Este ano'],...(allowHistorical?[["all",'Todo o período','Todo período'] as [Preset,string,string]]:[]),['custom','Período personalizado','Personalizado'],
      ...
      <div className="period-actions"><button onClick={()=>{const empty=allowHistorical?presetPeriod('all'):presetPeriod('operational',undefined,operationalStartDate(operationalStart));setDraft(empty);setStartText(isoToBrazilian(empty.start));setEndText(isoToBrazilian(empty.end));setError('')}}><X/> Limpar</button><button className="primary" onClick={apply}><Check/> Aplicar</button></div>
    </div>}
  </div>
}
