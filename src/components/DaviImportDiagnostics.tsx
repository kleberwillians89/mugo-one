...
import{analyzeCurrentCrmState,analyzeDaviFile,applySafeDaviDiagnostic,CrmFinding,CurrentCrmDiagnostic,DataWarning,DaviDiagnosticReport,DaviDiagnosticRow,DaviRowDivergence,safeDaviRows}from'../lib/davi-import-diagnostics'
...
export function DaviImportDiagnostics({onApplied=async()=>{}}:{onApplied?:()=>void|Promise<void>}){
 ...
 return <section className="davi-diagnostic" aria-label="Diagnóstico operacional do Davi Excel">
  <header><div><span>DIAGNÓSTICO OPERACIONAL</span><h2>Consistência do CRM</h2><p>Verifique a base atual ou analise uma nova planilha. Nenhuma dessas ações altera dados.</p></div><Database/></header>
  ...
  {report&&<div className="davi-diagnostic-results">
   ...
   <footer className="davi-diagnostic-actions"><div><strong>{safe.length} item(ns) seguro(s)</strong><span>Duplicidades, conflitos, inválidas e falhas de estoque nunca entram no apply.</span></div><button type="button" disabled={!safe.length||applying} onClick={()=>void applySafe()} title={safe.length?'Revalida e aplica atomicamente somente as linhas seguras.':'Nenhuma alteração segura para aplicar.'}>{applying?<><LoaderCircle className="spin"/>APLICANDO...</>:'APLICAR ALTERAÇÕES SEGURAS'}</button></footer>
  </div>}</details>
 </section>
}
