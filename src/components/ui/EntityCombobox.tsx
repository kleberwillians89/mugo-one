...
export function EntityCombobox({ value, onChange, search, placeholder, label, error, disabled=false, minimumCharacters=2, debounceMs=250, onCreate, createLabel, shouldOfferCreate, createActionPosition='bottom', noResultsLabel='Nenhum resultado encontrado.' }:{
  ...
  const id=useId(),request=useRef(0),[query,setQuery]=useState(value?.label??''),[editing,setEditing]=useState(false),[options,setOptions]=useState<EntityOption[]>([]),[open,setOpen]=useState(false),[loading,setLoading]=useState(false),[active,setActive]=useState(-1)
  useEffect(()=>{
    const term=query.trim(),current=++request.current
    if((value&&!editing)||term.length<minimumCharacters)return
  ...
  },[query,value,editing,search,minimumCharacters,debounceMs,onCreate])
  const choose=(option:EntityOption)=>{onChange(option);setEditing(false);setQuery(option.label);setOptions([]);setOpen(false);setActive(-1)}
  const clear=()=>{request.current++;onChange(null);setEditing(true);setQuery('');setOptions([]);setOpen(false);setActive(-1)}
