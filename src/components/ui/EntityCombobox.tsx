...
export function EntityCombobox({ value, onChange, search, placeholder, label, error, disabled=false, minimumCharacters=2, debounceMs=250 }:{
  value:EntityOption|null;onChange:(option:EntityOption|null)=>void;search:(query:string)=>Promise<EntityOption[]>;placeholder:string;label:string;error?:string;disabled?:boolean;minimumCharacters?:number;debounceMs?:number
}) {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>search(term).then(rows=>{if(request.current===current){setOptions(rows);setActive(rows.length?0:-1)}}).catch(()=>{if(request.current===current)setOptions([])}).finally(()=>{if(request.current===current)setLoading(false)}),debounceMs)
    return()=>window.clearTimeout(timer)
  },[query,value,search,minimumCharacters,debounceMs])
  ...
  const keyDown=(event:React.KeyboardEvent<HTMLInputElement>)=>{
    if(event.key==='ArrowDown'){event.preventDefault();setOpen(true);setActive(index=>nextComboboxIndex(index,'ArrowDown',options.length))}
    else if(event.key==='ArrowUp'){event.preventDefault();setActive(index=>nextComboboxIndex(index,'ArrowUp',options.length))}
    else if(event.key==='Enter'&&open&&active>=0){event.preventDefault();choose(options[active])}
    else if(event.key==='Escape'){event.preventDefault();setOpen(false);setActive(-1)}
  }
  const listId=`${id}-listbox`,status=query.trim().length<minimumCharacters?`Digite pelo menos ${minimumCharacters} caracteres.`:loading?'Buscando…':options.length?'':`Nenhum resultado para “${query.trim()}”.`
  return <div className={`entity-combobox ${error?'entity-combobox-error':''}`}>
    ...
    {open&&<div className="entity-combobox-popover"><div id={listId} role="listbox">{options.map((option,index)=><button id={`${id}-option-${index}`} role="option" aria-selected={value?.id===option.id} className={active===index?'active':''} type="button" key={option.id} onMouseDown={event=>event.preventDefault()} onMouseEnter={()=>setActive(index)} onClick={()=>choose(option)}><span><strong>{option.label}</strong>{option.description&&<small>{option.description}</small>}</span>{value?.id===option.id&&<Check aria-hidden="true"/>}</button>)}</div>{status&&<p role="status">{status}</p>}</div>}
