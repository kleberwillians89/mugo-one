...
export function SettingsTabs({ active }: { active: 'frete' | 'equipe' | 'entradas-de-leads' | 'comunicacoes' | 'fiscal' | 'cobrancas' }) {
  ...
  const canFiscal = useHasPermission('fiscal.manage')
  const canCollections = useHasPermission('collections.configure')
  ...
  if (!canTeam && !canLeadIntake && !canCommunications && !canFiscal && !canCollections) return null
  return (
    <div className="team-tabs">
      ...
      {canFiscal && <button className={active === 'fiscal' ? 'active' : undefined} onClick={() => go('/configuracoes/fiscal')}>Fiscal</button>}
      {canCollections && <button className={active === 'cobrancas' ? 'active' : undefined} onClick={() => go('/configuracoes/cobrancas')}>Cobranças</button>}
    </div>
  )
}
