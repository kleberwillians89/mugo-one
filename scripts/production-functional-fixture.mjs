...
try{
  ...
  state.ids.shipment=await rest('/rpc/create_draft_shipment',{method:'POST',body:JSON.stringify({p_client_id:client.id,p_allocation_ids:[allocation.id],p_notes:'Teste funcional calculator only'})},userHeaders)
  fs.mkdirSync('private_data',{recursive:true});fs.writeFileSync(stateFile,JSON.stringify(state,null,2),{mode:0o600})
  console.log(JSON.stringify({ready:true,client_id:client.id,shipment_id:state.ids.shipment,allocation_status:'shipping',settings:'controlled_transient',external_calls:0,credentials_file:stateFile},null,2))
