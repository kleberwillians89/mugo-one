...
try{
  ...
  const [sale]=await rest('/sales?select=id',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({organization_id:organizationId,client_id:client.id,perfume_id:perfume.id,sale_date:'2026-07-01',amount:250,payment_status:'paid',payment_method:'PIX',paid_at:'2026-07-01',source:'spreadsheet',sale_type:'SPLIT',volume_ml:10,volume_ml_raw:'10',perfume_name_raw:'PERFUME TESTE FUNCIONAL',inventory_allocation_eligible:false,created_by:user.id})},userHeaders);state.ids.sale=sale.id
  fs.mkdirSync('private_data',{recursive:true});fs.writeFileSync(stateFile,JSON.stringify(state,null,2),{mode:0o600})
  console.log(JSON.stringify({ready:true,client_id:client.id,sale_id:sale.id,historical_without_allocation:true,settings:'controlled_transient',external_calls:0,credentials_file:stateFile},null,2))
