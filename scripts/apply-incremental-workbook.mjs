...
const approved=staged.rows.filter((row)=>['existing_exact','existing_changed','new_safe','possible_duplicate','review_required'].includes(row.classification))
if(approved.length!==staged.report.total_lines)throw new Error('Staging incompleto.')
const summary={source:path.basename(input),classification:{existing_exact:staged.report.existing_exact,existing_changed:staged.report.existing_changed,new_safe:staged.report.new_safe,possible_duplicate:staged.report.possible_duplicate,review_required:staged.report.review_required},delta:staged.report.delta,mode}
